...
public class DBUserStorage implements UserStorage {
    ...
    public User get(int userId) {
        ...
        String query = "SELECT u.id, u.email, u.login, u.name, u.birthday WHERE u.id = ?;";
    ...
    public User update(User user) {
        ValidatorUser.validate(user);
        String query = "UPDATE films SET email = ?, login = ?, name = ?, birthday = ? WHERE id = ?;";
    ...
    public ArrayList<User> getAll() {
        String query = "SELECT u.id, u.email, u.login, u.name, u.birthday WHERE u.id = ?;";
        return (ArrayList<User>) jdbcTemplate.query(query, (rs, rowNum) -> makeUser(rs));
    }

    @Override
    public Map<Integer, User> getUsersMap() {
        ArrayList<User> usersList = getAll();
    ...
    public List<Integer> getUserFriends(int userId) {
        ValidatorUserId.validate(getUsersMap(), userId);
        String query = "SELECT f.friend_id, FROM friendships AS f WHERE f.user_id = ?;";
